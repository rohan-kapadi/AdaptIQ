...
function Profile() {
  ...
  const fetchPosition = async () => {
    try {
      ...
      const response = await api.get(`/api/user/job?email=${encodeURIComponent(user.email)}`);
  ...
  const fetchSkills = async () => {
    try {
      ...
      const response = await api.get(`/api/skills?email=${encodeURIComponent(user.email)}`);
...
const fetchTagline = async () => {
  try {
    ...
    const response = await api.get(`/api/user/tagline?email=${encodeURIComponent(user.email)}`);
...
const saveTagline = async () => {
  try {
    ...
    await api.post('/api/user/tagline', {
      email: user.email,
      tagline: tagline
    });
